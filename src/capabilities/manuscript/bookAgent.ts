...
export function createBookAgentSystemRulesText(bookPath: string): string {
  return [
    ...
    "## 故事圣经协作规则",
    "",
    "- 故事圣经用于维护人物、地点和规则条目；启用后内容位于 `lore/characters/`、`lore/locations/`、`lore/rules/` 和 `lore/tags/`。",
    "- 如果当前项目尚未启用故事圣经，先请用户运行 `LoreDock：启用故事圣经`，不要自行创建 `lore/` 目录结构。",
    "- 创建角色或人物条目时，优先使用 `LoreDock：新建人物`（命令 ID：`loredock.storyBible.createCharacter`）。",
    "- 创建地点或规则条目时，分别使用 `LoreDock：新建地点`（`loredock.storyBible.createLocation`）和 `LoreDock：新建规则`（`loredock.storyBible.createRule`）。",
    "- 从正文选中文本生成故事圣经条目时，使用 `LoreDock：从选中文本创建条目`（`loredock.storyBible.createCardFromSelection`），并让用户确认类型、名称和预览。",
    "- 不要直接伪造或改写故事圣经条目的结构字段，例如 `id`、`type`、`tags[0]`、`createdAt` 和 `updatedAt`；这些字段由 LoreDock 生成和维护。",
    "- 如果外置 AI 无法调用 LoreDock 命令，应先整理候选名称、摘要、别名、关键词和章节引用，请用户通过 LoreDock 创建条目，再协助补全文本内容。",
